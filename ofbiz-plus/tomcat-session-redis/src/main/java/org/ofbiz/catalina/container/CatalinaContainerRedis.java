 ...
 *
 */
public class CatalinaContainerRedis implements Container {
	...
	public static final String module = CatalinaContainerRedis.class.getName();
	...
	public void init(String[] args, String name, String configFile) throws ContainerException {
		...
		this.delegator = DelegatorFactory
				.getDelegator(ContainerConfig.getPropertyValue(cc, "delegator-name", "default"));
		...
		for (Connector con : tomcat.getService().findConnectors()) {
			...
			if (ph instanceof Http11Protocol) {
				...
				Debug.logInfo("Connector " + hph.getName() + " @ " + hph.getPort() + " - "
						+ (hph.getSecure() ? "secure" : "not-secure") + " [" + con.getProtocolHandlerClassName()
						+ "] started.", module);
			} else {
				Debug.logInfo("Connector " + con.getProtocol() + " @ " + con.getPort() + " - "
						+ (con.getSecure() ? "secure" : "not-secure") + " [" + con.getProtocolHandlerClassName()
						+ "] started.", module);
			}
		}
	...
	protected Engine createEngine(ContainerConfig.Container.Property engineConfig) throws ContainerException {
		...
		if (UtilValidate.isNotEmpty(clusterProps)) {
			...
			// createCluster(clusterProp, host); //redis
			clusterConfig.put(engineName, clusterProp);
		}

		// configure the CrossSubdomainSessionValve
		boolean enableSessionValve = ContainerConfig.getPropertyValue(engineConfig, "enable-cross-subdomain-sessions",
				false);
		...
		if (logDir != null) {
			...
			if (!logFile.isDirectory()) {
				throw new ContainerException(
						"Log directory [" + logDir + "] is not available; make sure the directory is created");
			}
	...
	protected Cluster createCluster(ContainerConfig.Container.Property clusterProps, Host host)
		...
		// ContainerConfig.getPropertyValue(clusterProps, "expire-session",
		// false);
	...
	protected Connector createConnector(ContainerConfig.Container.Property connectorProp) throws ContainerException {
		...
		// + ClassLoaderContainer.portOffset; //since 13.04
		int port = ContainerConfig.getPropertyValue(connectorProp, "port", 0);
		try {
			if (ClassLoaderContainer.class.getDeclaredField("portOffset") != null) {
				int portOffset = ClassLoaderContainer.class.getField("portOffset").getInt(null);
				port += portOffset;
			}
		...
		if (UtilValidate.isNotEmpty(connectorProp.properties)) {
			...
			Debug.logInfo("Creating connector for address='" + ((address == null) ? "ALL" : address) + "' port='" + port
	...
	private StandardContext configureContext(Engine engine, Host host, ComponentConfig.WebappInfo appInfo)
			throws ContainerException {
		// webapp settings
		...
		// } catch (Exception exc) {
		// throw new ContainerException(
		// "Cluster configuration requires a valid manager-class property: " +
		// exc.getMessage());
		// }
		// } else {
		...
		context.addValve(redusValve); // redis
	...
	protected void loadComponents() throws ContainerException {
		...
		ScheduledExecutorService executor = ExecutionPool.getExecutor(CATALINA_THREAD_GROUP, "catalina-startup", -1,
				true);
		try {
			...
			for (int i = webResourceInfos.size(); i > 0; i--) {
				...
				if (!keys.removeAll(loadedMounts)) {
					...
					appInfo.appBarDisplay = false; // disable app bar display on
													// overrided apps
					Debug.logInfo("Duplicate webapp mount; not loading : " + appInfo.getName() + " / "
							+ appInfo.getLocation(), module);
				}
			}
	...
	public void stop() throws ContainerException {
		try {
			...
			// process
			Debug.logVerbose(e, module); // happens usually when running tests,
											// disabled unless in verbose
		}
	}
	...
	protected void configureMimeTypes(Context context) throws ContainerException {
		Map<String, String> mimeTypes = CatalinaContainerRedis.getMimeTypes();
