...
public abstract class AbstractEntityConditionCache<K, V> extends AbstractCache<K, V> {
	...
	public void remove(GenericEntity entity) {
		UtilRedisCache.clearCache(getCacheName(entity.getEntityName()));
	...
	protected ModelEntity getModelCheckValid(GenericEntity oldEntity, GenericEntity newEntity) {
		...
		if (!isNull(newEntity)) {
			...
			if (oldEntity != null && !entityName.equals(oldEntity.getEntityName())) {
				throw new IllegalArgumentException("internal error: storeHook called with 2 different entities(old="
						+ oldEntity.getEntityName() + ", new=" + entityName + ")");
			}
		} else {
			if (!isNull(oldEntity)) {
	...
	public void storeHook(boolean isPK, GenericEntity oldEntity, GenericEntity newEntity) {
		...
		if (newEntity == null) {
			// Debug.logInfo("In storeHook calling sub-storeHook for entity name
			// [" + entityName + "] for the oldEntity: " + oldEntity, module);
		}
		storeHook(entityName, isPK, UtilMisc.toList(oldEntity), UtilMisc.toList(newEntity));
		Iterator<String> it = model.getViewConvertorsIterator();
		while (it.hasNext()) {
			String targetEntityName = it.next();
			storeHook(targetEntityName, isPK, convert(isPK, targetEntityName, oldEntity),
					convert(false, targetEntityName, newEntity));
		}
	}

	protected <T1 extends Map<String, Object>, T2 extends Map<String, Object>> void storeHook(String entityName,
			boolean isPK, List<T1> oldValues, List<T2> newValues) {
		UtilRedisCache<EntityCondition, Map<K, V>> entityCache = UtilRedisCache.findCache(getCacheName(entityName));
		// for info about cache clearing
		if (UtilValidate.isEmpty(newValues) || newValues.get(0) == null) {
			// Debug.logInfo("In storeHook (cache clear) for entity name [" +
			// entityName + "], got entity cache with name: " + (entityCache ==
			// null ? "[No cache found to remove from]" :
			// entityCache.getName()), module);
		}
		if (entityCache == null) {
			return;
		}
		for (EntityCondition condition : entityCache.getCacheLineKeys()) {
			...
			if (condition == null) {
				...
				while (oldValueIter.hasNext() && !shouldRemove) {
					...
					if (condition.mapMatches(getDelegator(), oldValue)) {
						...
						// entityName [" + entityName + "]; shouldRemove is
						// false", module);
						if (newValues != null) {
							Iterator<T2> newValueIter = newValues.iterator();
							while (newValueIter.hasNext() && !shouldRemove) {
								...
								// [" + entityName + "] shouldRemove is now " +
								// shouldRemove, module);
							}
						} else {
							shouldRemove = true;
				...
				if (!oldMatched && isPK) {
					// Debug.logInfo("In storeHook, for entityName [" +
					// entityName + "] oldMatched is false and isPK is true, so
					// setting shouldRemove to true (will remove from cache)",
					// module);
					shouldRemove = true;
				}
			}
			if (shouldRemove) {
				if (Debug.verboseOn())
					Debug.logVerbose("In storeHook, matched condition, removing from cache for entityName ["
							+ entityName + "] in cache with name [" + entityCache.getName() + "] entry with condition: "
							+ condition, module);
				// doesn't work anymore since this is a copy of the cache
				// keySet, can call remove directly though with a concurrent mod
